 ...
 then
  ...
  echo "${j}"
    ...
    then
        ...
        printf "#!/bin/bash\n\n /bin/bash ${DB}/scripts/run_bowtie2_blca.sh -o ${OUT} -d ${DB} -m ${j} -l -b ${B_VALUE:=$BLCAB} -c ${BCC_CUT_OFF:=$DEF_BCC_CUT_OFF} -n ${BOOT:=$BOOTSTRAP} -x ${MATCH:=$MUSMATCH} -f ${MISMATCH:=$MUSMISMATCH} -g ${GAPP:=$MUSGAPP} -k ${HPC_HEADER:=$HPC_HEADER_FILE}\n\necho _END_ [run_bowtie2_blca.sh]" > ${OUT}/Run_info/run_scripts/${j}_bowtie2_blca_job.sh
