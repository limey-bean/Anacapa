...
# ~/Anacapa_db/anacapa_QC_dada2.sh -i <input_dir> -o <out_dir> -d <database_directory> -u <hoffman_account_user_name> -f <fasta file of forward primers> -r <fasta file of reverse primers> -a <adapter type (nextera or truseq)>  -t <illumina run type HiSeq or MiSeq>
IN=""
OUT=""
DB=""
UN=""
...
while getopts "i:o:d:u:f:r:a:t:" opt; do
    case $opt in
        ...
        ;;
        t) ILLTYPE="$OPTARG"  #need to know trim parameters for cutadapt
        ;;
    esac
done
...
${ATS} #load ATS, a Hoffman2-specific module for managing submitted jobs
date
###
...
# Generate cutadapt primer files -> merge reverse complemented primers with adapters for cutting 3'end sequencing past the end of the metabarcode region, and add cutadapt specific characters to primers and primer/adapter combos so that the appropriate ends of reads are trimmed.
...
cp ${DB}/adapters_and_PrimAdapt_rc/*_${ADPT}_*_adapter.txt ${OUT}/Run_info/cutadapt_primers_and_adapters  # make a copy of the appropriate adapter file in your output directory
...
# now use the formatted cutadapt primer file to trim fastq reads
...
do
# first chop off the 5' adapter and 3' adapter and primer combo (reverse complemented)
 str1=${str%_*}
 j=${str1#${OUT}/QC/fastq/}
 echo ${j} "..."
 ...
 # stringent quality filter to get rid of the junky reads. It chops the low quality reads off of the ends. See the documentation for details. The default average quality score for retained bases is 35 and the minimum length is 100.  Any reads that do not meet those criteria are removed
 ...
 fastq_quality_trimmer -t ${MIN_QUAL} -l ${MIN_LEN}  -i ${OUT}/QC/cutadapt_fastq/untrimmed/${j}_Paired_2.fastq -o ${OUT}/QC/cutadapt_fastq/${j}_qcPaired_2.fastq -Q33 #trim pair two
 rm ${OUT}/QC/cutadapt_fastq/untrimmed/${j}_Paired_2.fastq # remove intermediate files
 # sort by metabarcode but run additional trimming.  It makes a difference in merging reads in dada2.  Trimming varies based on sequencing platform.
 echo "forward..."
 if [ "${ILLTYPE}" == "MiSeq"  ]; # if MiSeq chops more off the end than if HiSeq, modify length in the vars file
 then
   # use cutadapt to search the 5' end of forward reads for forward primers.  These are then sorted by primer name.  We do an additional trimming step analagous to the trimming step in the dada2 tutorial.  Because the forward reads tend to be higher quality we only trim 20 bp from the end by default for the MiSeq (longer reads). Users can modify all parameters in the vars file.
  ...
  echo "check"
  echo "reverse..."
  # use cutadapt to search 5' end of reverse reads for reverse primers.  These are then sorted by primer name.  We do an additional trimming step analagous to the trimming step in the dada2 tutorial.  Because these reverse reads tend to be lower quality we only trim 50 bp from the end by default for the MiSeq (longer reads). Users can modify all parameters in the vars file.
  ...
  rm ${OUT}/QC/cutadapt_fastq/${j}_qcPaired_2.fastq # remove intermediate files
   ...
   # use cutadapt to search 5' end of forward reads for forward primers.  These are then sorted by primer name.  We do an additional trimming step analagous to the trimming step in the dada2 tutorial.  Because these forward reads tend to be higher quality we only trim 10 bp from the end by default for the MiSeq (shorter reads). Users can modify all parameters in the vars file.
  ...
  echo "check"
  echo "reverse..."
  # use cut adapt to search 5' end of reverse reads for reverse primers.  These are then sorted by primer name.  We do an additional trimming step analagous to the trimming step in the dada2 tutorial.  Because these reverse reads tend to be lower quality we only trim 25 bp from the end by default for the MiSeq (shorter reads). Users can modify all parameters in the vars file.
...
do
 ...
 then
    ...
    echo $(DADA2_PAIRED_TEMPLATE) > ${OUT}/Run_info/hoffman2/run_scripts/${j}_dada2_paired_job.sh
    echo $(DADA2_PAIRED_F_TEMPLATE) > ${OUT}/Run_info/hoffman2/run_scripts/${j}_dada2_F_job.sh
    echo $(DADA2_PAIRED_R_TEMPLATE) > ${OUT}/Run_info/hoffman2/run_scripts/${j}_dada2_R_job.sh
    echo ''
