 ...
 then
  ...
  echo "${j}"
    ...
    then
        ...
        printf "#!/bin/bash\n\nsh ${DB}/scripts/run_dada2.sh  -o ${OUT} -d ${DB} -m ${j} -t paired -e ${MIN_MERGE_LENGTH:=$DEF_MIN_LENGTH} \n" > ${OUT}/Run_info/run_scripts/${j}_dada2_paired_job.sh
        printf "#!/bin/bash\n\nsh ${DB}/scripts/run_dada2.sh  -o ${OUT} -d ${DB} -m ${j} -t forward -e ${MIN_MERGE_LENGTH:=$DEF_MIN_LENGTH} \n" > ${OUT}/Run_info/run_scripts/${j}_dada2_F_job.sh
        printf "#!/bin/bash\n\nsh ${DB}/scripts/run_dada2.sh  -o ${OUT} -d ${DB} -m ${j} -t reverse -e ${MIN_MERGE_LENGTH:=$DEF_MIN_LENGTH} \n" > ${OUT}/Run_info/run_scripts/${j}_dada2_R_job.sh
        bash ${OUT}/Run_info/run_scripts/${j}_dada2_paired_job.sh
        date
        bash ${OUT}/Run_info/run_scripts/${j}_dada2_F_job.sh
        date
        bash ${OUT}/Run_info/run_scripts/${j}_dada2_R_job.sh
        date
    else
