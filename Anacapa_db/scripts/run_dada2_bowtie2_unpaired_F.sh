...
rm -r ${OUT}/unpaired_1/${MB}/filtered
...
### end to end mode
bowtie2 -x ${DB}/${MB}/${MB}_bowtie2_database/${MB}_bowtie2_index  -f -U ${OUT}/dada2_out/unpaired_F/${MB}/nochim_forward${MB}.fasta -S ${OUT}/bowtie2_runs/unpaired_F/${MB}/nochim_forward_only${MB}_end_to_end.sam --no-hd --no-sq -D 20 -R 3 -N 0 -L 15  -i S,1,0.50 --end-to-end --no-unal -p 120 -k 100 --un ${OUT}/dada2_out/unpaired_F/${MB}/nochim_forward${MB}_end_to_end_reject.fasta --omit-sec-seq

### local mode

bowtie2 -x ${DB}/${MB}/${MB}_bowtie2_database/${MB}_bowtie2_index  -f -U ${OUT}/dada2_out/unpaired_F/${MB}/nochim_forward${MB}_end_to_end_reject.fasta -S ${OUT}/bowtie2_runs/unpaired_F/${MB}/nochim_forward_only${MB}_local.sam --no-hd --no-sq -D 20 -R 3 -N 0 -L 15  -i S,1,0.50 --local --no-unal -p 120 -k 100 --un ${OUT}/dada2_out/unpaired_F/${MB}/nochim_forward${MB}_end_to_end_and_local_reject.fasta --omit-sec-seq
