...
class SummaryAppender(object):
    def __init__(self, sam_file_names, taxonomy_file_name):
        ...
        for sam_file_name in sam_file_names:
            with open(sam_file_name) as sam_file:
                self.parsed_bowtie_output.update(self.parse_bowtie_output(sam_file))
    ...
    def parse_bowtie_output(self, sam_file):
        summary = {}
        previous_entries = []
        ...
        for line in sam_file:
            ...
            if len(previous_entries) == 0 or previous_entries[0].qname == entry.qname:
                ...
                summary[previous_entries[0].qname] = self.handle_group_of_entries(previous_entries)
    ...
    def handle_group_of_entries(self, entries):
        ...
        taxonomies = [self.taxonomy[entry.rname] for entry in entries]
        taxonomies_in_range = [self.taxonomy[entry.rname] for entry in entries if entry.identity_ratio >= id_cutoff]
        taxonomy_consensus = self.determine_taxonomy_consensus(taxonomies)
    ...
    def append_to_summary(self, summary_file_name):
        fields = ['percent_ids','percent_id_bin','single_or_multiple_hit', 'length_of_hit', 'overhang_or_soft_clipping',
                  'consensus_taxonomic_path', 'taxonomic_paths_above_cutoff']
        ...
        new_summary_file.write('\t'.join([current_header] + fields) + '\n')
        for line in current_summary_file[1:]:
            ...
            if name in self.parsed_bowtie_output:
                ...
                new_summary_file.write('\t'.join([line] + ordered_sam_info) + '\n')
            else:
                new_summary_file.write(line + '\t' * len(fields))
...
parser.add_argument('sam_files', nargs='+', type=str, help='The SAM files to be read')

if __name__ == '__main__':
    args = parser.parse_args()

    sam_file_names = args.sam_files
    summary_file_name = args.summary_file
    taxonomy_file_name = args.taxonomy_file
    appender = SummaryAppender(sam_file_names, taxonomy_file_name)
