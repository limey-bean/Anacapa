...
The last step of the Anacapa Pipeline conducts exploratory data analysis on the input sequences to provide a first pass look at the taxonomic assignments and generated data tables. This analysis is not meant for publication, but solely as a first stab at visualizing your data, identifying potential glaring errors or contamination, and identifying patterns worth investigating further. We highly encourage data exploration before further analysis as different parameters within the Anacapa pipeline may produce differences in downstream results and will vary by project and stringency of taxonomic assignment. The exploratory_analysis.R script uses a variety of R packages, relying heavily on phyloseq, vegan, and ggplot2. See below for full list of R package dependencies and scripts. The output from reformat_summary_for_R.py has a similar data structure to the BIOM format and is the input used for the exploratory_analysis.R script. In addition, the user supplies an input metadata table using QIIME sample metadata format. The first step of the R script is to convert the input files into a Phyloseq class object. We then generate bar plots looking at total number of observed classes and relative abundance of each class, faceting by each categorical metadata column. We then generate rarefaction curves, alpha diversity boxplots to observe total number of taxa and Shannon diversity, and alpha diversity statistics, faceting by each metadata column. In addition, we calculate jaccard and bray-curtis distance matrices and conduct NMDS ordination plots, network map, heat maps, and ward-linkage maps again faceting by each categorical metadata column. In addition we conduct two betadiversity statistical tests, pairwise adonis and betadisp from the vegan package, again faceting by each metadata column. 
