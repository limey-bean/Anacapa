...
# pdf("figures/heatmap-v1-noBlastBLCAs.pdf", height = 15, width = 22)
superheat.2(comparisons_df, membership.cols = rep(1:length(all_files_for_heatmap), each = 5), heat.pal = colors(7),
...
rownames(barplot_of_empties) <- c("correct", "wrong", "unable to call")
barplot_of_empties2 <- melt(barplot_of_empties)
barplot_of_empties2$db_and_percent <- rep(paste0(names(comparisons_e),"_", c(60,70,80,90,95)))
