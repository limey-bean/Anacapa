...
library(tidyverse)
library(superheat)
subdir_names <- list.files("data-for-figs/Database_comparisons/compare-18S-databases/18S_V8-9/", pattern = "coded")
file_names <- list.files("data-for-figs/Database_comparisons/compare-18S-databases/18S_V8-9/Silva_V8-9_18S_coded/")
all_files <- lapply(subdir_names, function(subdir) lapply(file_names, function(each_file) 
  read_delim(file.path("data-for-figs/Database_comparisons/compare-18S-databases/18S_V8-9/", subdir, each_file), delim = "\t") %>% 
...
group_names <- c("CRUX\nFiltered 18S V8-V9","CRUX\nUnfiltered 18S V8-9", "Silva")


colors <- colorRampPalette(c("white", "black"))
colors(6)
pdf("figures/heatmap-18s_V8-9.pdf", height = 15, width = 22)
...
write.csv(barplot_of_empties, "figures/accuracy_percentages_per_barcode/accuracy_18s_V8-9.csv")
colnames(barplot_of_empties) <- NULL


pdf("figures/18s_v8-9_database_accuracy_comparisons.pdf", width = 22, height = 7)
