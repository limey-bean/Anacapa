...
all_files <- lapply(subdir_names, function(subdir) lapply(file_names, function(each_file) 
  read_delim(file.path("data-for-figs/compare-18S-databases/18S_V8-9/", subdir, each_file), delim = "\t")))
names(all_files) <- subdir_names

mock <- read_delim("data-for-figs/compare-18S-databases/18S_V8-9/18S_V8-9_taxonomy.txt", delim ="\t")
...
pdf("figures/18S_V8-9_database_accuracy_comparisons.pdf")
...
pdf("figures/heatmap-18S_V8-9.pdf")
superheat(for_heatmap, membership.cols = rep(c("CRUX 18S V8-9\nFiltered","CRUX 18S V8-9\nUnfiltered",
                                               "Silva 18S V4"), each = 5), 
          heat.pal = colors(7),
          grid.vline.col = "white", grid.vline.size = 2, bottom.label.text.size = 7,
          bottom.label.size = .15,
